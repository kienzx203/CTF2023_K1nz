...
with sqlite3.connect('database.db') as con:
    cursor = con.cursor()
    cursor.execute(
        'CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY, username TEXT, password TEXT, sus BOOLEAN)')
...
def signup():
    with sqlite3.connect('database.db') as con:
        ...
        if cursor.fetchone():
            ...
            cursor.execute(
                'INSERT INTO users (username, password, sus) VALUES (?, ?, ?)', (username, password, sus))
            con.commit()
...
def login():
    with sqlite3.connect('database.db') as con:
        ...
        try:
            ...
            if user:
                randomToken = ''.join(random.choice(
