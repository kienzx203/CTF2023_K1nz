...
if (isset($_FILES) && !empty($_FILES)) {
  $uploadpath = "/var/tmp/"; // /var/tmp/BKSEC_guest_1692448643_file_example.jpg
  $error = "";

  $timestamp = time(); //thời gian nhận được file.

  $userValue = $_COOKIE['user']; //BKSEC_guest
  $target_file = $uploadpath . $userValue . "_" . $timestamp . "_" . $_FILES["image"]["name"]; //tên file nối chuỗi
  //var/tmp/../www/html/assets/images/_timestamp_tên file.

  //    /var/tmp/BKSEC_guest_1692448643_file_example.jpg  // /var/www/html/..........  

  // filephp      trong gửi file _+3  

  move_uploaded_file($_FILES["image"]["tmp_name"], $target_file);

  if ($_FILES["image"]["size"] > 1048576) {
    $error .= '<p class="h5 text-danger">Maximum file size is 1MB.</p>';
  } elseif ($_FILES["image"]["type"] !== "image/jpeg") { // Có thể sửa trong burpsuite
...
<body class="bg-body-tertiary">
  ...
  <div class="container">
    <main>
      ...
      echo $error;
      echo $metadata;
      unlink($target_file); // Xóa file luôn. 
      ?>
    </main>
